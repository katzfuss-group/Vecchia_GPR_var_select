                 ...
                 arg_check = function(){T}, silent = FALSE)
{
  ...
  for(i in 1 : maxIterOut)
  {
    ...
    DSCObj <- dsc_lb(H, b, parms, silent, convtolIn, maxIterIn, lb) # make sure DSCObj$parms is no smaller than lb
                 ...
                 1e-4))
    {
      ...
      if(!silent)
        cat("Lower bounded coordinate descent is used \n")
    }else # Use lower bounded gradient descent
    {
      parmsNew <- gd_Armijo(parms, obj, grad, 1e-4, 
                            function(x){- likfun(x)$loglik}, lb, arg_check)
      if(!silent)
        cat("Coordinate descent (not used) outputs:", DSCObj$parms, "\n",
            "Lower bounded gradient descent is used \n")
    }
