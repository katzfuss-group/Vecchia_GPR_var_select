...
This markdown is to show that the CQCD algorithm doesn't work well when `d` is relatively big, e.g., 100.
...
```{r save the result}
save.image(file = "CQCD_big_d.RData")
```
...
ggplot(data = mydf, aes(x = iter, y = val, group = varid, col = true_val, 
                        label = err)) +
  geom_line() +
  # geom_label(nudge_y = -0.16, show.legend = F) +
...
ggsave("CQCD_big_d.pdf", width = 6, height = 3.375)
```
