...
## Overview
This summary is based on `grad_info_sqrel` to show that using the BIC penalty outlined in `BIC penalty explain` can effectively deselect predictors.
```{r clean environment}
rm(list = ls())
```
...
for(i in 1 : 10)
{
  ...
  BIC_pen <- function(theta){
    log(n) / 2 * sum(pmin(theta[2 : (1 + dRel)] / 
                          (2 * thetaRel[2 : (1 + dRel)]), 1))
  }
  BIC_dpen <- function(theta){
    dpen <- c(0, log(n) / 2 * 
                (theta[2 : (1 + dRel)] < 2 * thetaRel[2 : (1 + dRel)]) / 
                (2 * thetaRel[2 : (1 + dRel)]), 0)
  ...
  cat("Opt with rel used", 
      ...
      "seconds\n")
  thetaRel <- optObj$covparms
  objNew <- - optObj$loglik - BIC_pen(thetaRel)
...
```{r save the result}
save.image(file = "grad_info_sqrel_pen.RData")
```
...
for(i in 1 : 10)
{
  ...
  mydf[(i - 1) * d + setdiff(rslt[[4 * (i - 1) + 2]], rslt[[4 * (i - 1) + 3]]), 
       1 : 2] <- NA
  mydf[(i - 1) * d + rslt[[4 * (i - 1) + 3]], 3] <- rslt[[4 * (i - 1) + 3]]
  mydf[(i - 1) * d + which(rSq > 0), 4] <- which(rSq > 0) # color
}
mydf <- as.data.frame(mydf)
colnames(mydf) <- c("gradient", "iter", "label", "color")
mydf <- mydf[!is.na(mydf$gradient), ]
mydf$iter <- as.factor(mydf$iter)
mydf$color[mydf$color > 0] <- paste0("var ", mydf$color[mydf$color > 0])
mydf$color[mydf$color == 0] <- "irrelevant var"
...
ggsave("gradient_sr_pen.pdf", width = 6, height = 3.375)
```
...
for(i in 1 : 10)
{
  ...
  if(i < 6)
  {
    ...
    mydf[rowIdx + length(idxVar), 6] <- sqrt(rSq[i]) # true r for y intercept
  }
...
ggplot(data = mydf, aes(x = iter, y = val, group = varid, col = label,
  ...
  geom_hline(mapping = aes(col = label, yintercept = y_intercept), 
             size = 0.5, lty = "dashed") + 
  theme(legend.title = element_blank(), legend.position="right")
ggsave("fwdbwd_conv_pen.pdf", width = 6, height = 3.375)
```
