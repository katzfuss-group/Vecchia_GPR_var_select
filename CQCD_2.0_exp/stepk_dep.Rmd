...
```{r save the result}
save.image(file = paste0("step", k, "_dep.RData"))
```
...
ggsave(paste0("gradient_dep_", k, ".pdf"), width = 6, height = 3.375)
```
...
ggplot(data = mydf, aes(x = iter, y = val, group = varid, col = label,
  ...
  geom_hline(mapping = aes(col = label, yintercept = y_intercept), 
             size = 0.5, lty = "dashed") + 
  theme(legend.title = element_blank(), legend.position="right")
ggsave(paste0("fwdbwd_dep_", k, ".pdf"), width = 6, height = 3.375)
```
