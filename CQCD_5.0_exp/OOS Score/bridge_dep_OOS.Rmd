...
CV_score <- function(theta)
{
  ...
  locsRel <- locsOOS[, idxLocsRel, drop = F]
  ...
  - vecchia_profbeta_loglik(thetaRel, "matern25_scaledim_sqrelevance",
                            yOOS, XOOS, locsRel, NNarray)$loglik
}
```
                                ...
                                gradObj0)
{
  ...
  while(T)
  {
    ...
    thetaRel[(length(idx) + 2) : (length(idx) + k + 1)] <- 1e-8
...
save(list = c("d", "idxSet", "thetaSet", "scoreSet", "lambdaSet", "loglikSet",
              "lambdaVec", "n", "d"), 
     file = paste0("bridge_dep_OOS_", n, "_", d, ".RData"))
```
...
ggsave(paste0("bridge_dep_OOS_", n, "_", d, ".pdf"), width = 7, height = 5)
```
