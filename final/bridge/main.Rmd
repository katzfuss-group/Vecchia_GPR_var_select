...
for(i in 1 : length(lambVec)){
  ...
  optObj <- fwd_bwd(idx = idx, theta = theta, sr0 = sr0, locsIn = locsIn, 
                    ...
                    ddpen_fun = ddpen_fun, OOS_score = OOS_crps, 
                    stop_con_fb = stop_con_OOS1_fb, miniCQCD = NULL, 
                    miniGrad = NULL, convCQCD = convCQCD, 
...
save(list = c("n", "d", "rho", "idxSet", "thetaSet", "scrVec", "lambVec"), 
     file = paste0("GPBrg_", n, "_", d, "_", rho, ".RData"))
```
...
firstIdx <- max(which(sapply(thetaSet, 
                             function(x){sum(x[2 : (d + 1)]) > 1e-4}))[1] - 1, 
                1)
...
lambStop <- lambVec[which(scrChg < 0)[1] + firstIdx - 1]
...
if(rho != 0){
  figFn <- paste0("CQCD_bridge_dep_", n, "_", d, ".pdf")
}else{
  figFn <- paste0("CQCD_bridge_", n, "_", d, ".pdf")
}
