...
nIter <- 100
result <- array(dim = c(nIter, length(batchSzVec), d + 2, 4), 
                dimnames = list(paste("Iter", 1 : nIter), 
                                paste("Batchsize", batchSzVec),
                                c("Variance", paste("SR", 1 : d), "Nugget"),
                                paste("Method", 1 : 4)))
for(j in 1 : length(batchSzVec)){
  batchSz <- batchSzVec[j]
  for(i in 1 : nIter){
    result[i, j, , 1] <- mtd1(batchSz) / batchSz - gradFull / n
    result[i, j, , 2] <- mtd2(batchSz) / batchSz - gradFull / n
    result[i, j, , 3] <- mtd3(batchSz) / batchSz - gradFull / n
    result[i, j, , 4] <- mtd4(batchSz) / batchSz - gradFull / n
  }
...
for(i in 1 : (d + 2)){
  ...
  for(j in 1 : 4){ # methods
    for(k in 1 : length(batchSzVec)){ # batch size
      ...
      df[(j - 1) * length(batchSzVec) + k, 3] <-
        sd(result[, k, i, j]) / sqrt(nIter)
      df[(j - 1) * length(batchSzVec) + k, 4] <- j
    }
  }
  colnames(df) <- c("batchsz", "bias", "bias_sd", "mtdID")
  df <- as.data.frame(df)
  ...
  plt <- ggplot(data = df, aes(x = batchsz, y = bias_sd, col = mtdID)) +
    geom_line() +
    scale_x_continuous(trans = pseudo_log_trans(sigma = 1),
                       breaks = batchSzVec) +
    scale_color_manual(breaks = unique(df$mtdID),
  ...
  if(i > 1 && i < d + 2)
    plt <- plt + scale_y_continuous(trans = pseudo_log_trans(sigma = 0.01))
  ggsave(paste0("grad_bias_sd_", varName, ".pdf"), plot = plt,
         width = 7, height = 5)
}
