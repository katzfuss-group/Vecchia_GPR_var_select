                 ...
                 arg_check = function(){T}, silent = FALSE)
{
  ...
  for(i in 1 : maxIterOut)
  {
    ...
    if(arg_check(DSCObj$parms) && 
       is_Armijo(DSCObj$parms, - likfun(DSCObj$parms)$loglik, parms, obj, grad, 
                 1e-4))
    {
      parmsNew <- DSCObj$parms
      if(!silent)
        cat("FIM is used \n")
    }else # Use BFGS approx of Hessian
    {
      b <- grad - as.vector(B %*% parms)
      DSCObj <- dsc_lb(B, b, parms, silent, convtolIn, maxIterIn, lb)
      parmsNew <- bktk_Armijo(parms, obj, grad, DSCObj$parms, 1e-4, 
                              function(x){- likfun(x)$loglik}, lb,
                              arg_check)
      if(all(parmsNew == parms)) # Use gradient descent
      {
    ...
    # Gradients, FIM, and B at the new parms
...
gd_Armijo <- function(parms, obj, grad, c, obj_func, lb, arg_check)
{
  ...
  cat(paste("gd_Armijo cannot find a new parameter vector that satisfies",
