---
title: "Square-root Log-penalty GP Regression with Warm Start"
...
dpenfun <- function(theta){
  r <- theta[-c(1, length(theta))]
  rExp <- exp(- sqrt(r))
  rpen <- lambda * rExp / (1 + rExp)^2 / sqrt(r) / 2
  rpen[r < 1e-10] <- 0
  c(0, rpen, 0)
}
...
ggsave("sqrtLog_warm_rev.pdf", width = 7, height = 5)
```
