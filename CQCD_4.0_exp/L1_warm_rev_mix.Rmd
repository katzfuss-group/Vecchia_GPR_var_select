...
fit_order_select_SR <- function(theta0, idx0, loglik0)
{
  ...
  while(T)
  {
    ...
    optObj$covparms[2 : (length(idxNew) + 1)] <- 
      sqrt(optObj$covparms[2 : (length(idxNew) + 1)])
    # opt with Rel parameters
    objfun <- function(theta){
      likObj <- GpGp::vecchia_profbeta_loglik(theta, 
    ...
    optObj <- CQCD(objfun, objfun_gdfm, optObj$covparms, maxIterOut = 15, 
                   convtolOut = 1e-3,
                   maxIterIn = 40, lb = lbRel, arg_check = arg_check_R)
    optObj$covparms[2 : (length(idxNew) + 1)] <- 
      (optObj$covparms[2 : (length(idxNew) + 1)])^2
  ...
  cat("lambda =", lambda, "idx =", idx, "theta =", theta, "\n\n")
...
lambdaVec <- rev(c(0, 1, 5, 10, 20, seq(from = 50, to = 1e3, by = 50)))
...
ggsave("L1_warm_rev_mix.pdf", width = 7, height = 5)
```
