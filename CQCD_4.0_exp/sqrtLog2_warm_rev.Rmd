---
title: "Square-root Log-penalty V2.0 GP Regression with Warm Start"
...
penfun <- function(theta){
  ...
  rExp <- exp(- lambda * sqrt(r))
  sum(lambda / (1 + rExp) - lambda / 2)
}
dpenfun <- function(theta){
  r <- theta[-c(1, length(theta))]
  rExp <- exp(- lambda * sqrt(r))
  rpen <- lambda^2 * rExp / (1 + rExp)^2 / sqrt(r) / 2
  rpen[r < 1e-10] <- 0
  c(0, rpen, 0)
}
...
ggsave("sqrtLog2_warm_rev.pdf", width = 7, height = 5)
```
