...
OOS_score <- function(theta)
{
  ...
  NNarray <- get.knnx(locsRelScal[(n + 1) : (n + nOOS), , drop = F], 
                      locsRelScal[1 : n, , drop = F], m + 1)$nn.index
...
lambdaVec <- rev(c(0, kronecker(10^(-1 : max(round(log10(n)) - 1, 1)), 
                                c(1, 2, 5))))
...
ggplot(data = dfScore, aes(x = lambda, y = score)) + 
  ...
  scale_y_continuous(trans = pseudo_log_trans(sigma = 0.1),
                     breaks = lambdaVec) +
  theme(legend.position = "none")
ggsave(paste0("CQCD_bridge_OOS_dep_", n, "_", d, ".pdf"), width = 7, height = 5)
```
