...
rm(covM, cholM)
```
A list of theta, where the gradients are computed.
```{r theta list, eval = T}
thetaList <- list()
thetaList[[1]] <- c(sigmasq, rep(1e-8, d), tausq)
for(l in 2 : d0)
  thetaList[[l]] <- c(sigmasq, rSq[1 : (l - 1)], rep(0, d + 1 - l), tausq)
```
...
for(i in 1 : length(thetaList))
{
  ...
  odr <- order_maxmin(locsScal[, 1 : d0, drop = F])
  yOdr <- y[odr]
  locsOdr <- locs[odr, , drop = F]
  locsScalOdr <- locsScal[odr, , drop = F]
  NNarray <- find_ordered_nn(locsScalOdr[, 1 : d0, drop = F], m = m)
...
dfGrad <- matrix(NA, d * d0 * 2, 3)
dfGrad[, 1] <- kronecker(c(1 : (d0 * 2)), rep(1, d))
dfGrad[, 2] <- c(unlist(lapply(gradList, function(x){x / max(abs(x))})),
                 unlist(lapply(gradDepList, function(x){x / max(abs(x))})))
...
ggplot(data = dfGrad, aes(x = ID, y = grad, col = col)) +
  ...
  scale_color_manual(breaks = unique(dfGrad$col),
                     values = c(rainbow(d0), rep("grey", d - d0))) +
  geom_vline(xintercept = d0 + 0.5, col = "red", lty = "dashed") +
  xlab("# Selected Predictors (Left: independent | Right: dependent)") + 
...
dfGrad <- matrix(NA, d * d0 * 2, 3)
dfGrad[, 1] <- kronecker(c(1 : (d0 * 2)), rep(1, d))
dfGrad[, 2] <- c(unlist(lapply(gradMinibatList, function(x){x / max(abs(x))})),
                 unlist(lapply(gradMinibatDepList, function(x){x / max(abs(x))})))
dfGrad[, 3] <- kronecker(rep(1, d0 * 2), c(1 : d0, rep(d0 + 1, d - d0)))
colnames(dfGrad) <- c("ID", "grad", "col")
dfGrad <- as.data.frame(dfGrad)
dfGrad$col <- as.factor(dfGrad$col)
