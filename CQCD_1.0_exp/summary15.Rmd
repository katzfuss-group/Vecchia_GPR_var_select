...
Different from `summary14`, `quad_cdsc_SCAD` is used instead of `quad_cdsc_L1`.
...
## Experiment
`quad_cdsc_L1_brute` computes gradients w.r.t. zero parameters as well while `quad_cdsc_SCAD` ignores zero parameters.
...
while(length(idx) <= d - k)
{
  ...
  optObj <- quad_cdsc_SCAD(objfun, objfun_gdfm, locsOdr, 1, thetaTmp, 1e-3, silent = T,
