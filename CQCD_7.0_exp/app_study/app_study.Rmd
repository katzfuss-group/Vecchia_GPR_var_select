...
dataFn <- "Sydney_Data_processed.csv"
...
objPred <- predict(pcrMdl, dataSetTest[, 1 : d], ncomp = 20)
sum((objPred - dataSetTest[, d + 1])^2)
```
Compare with using just mean
```{r compare with using just mean, eval = T}
var(dataSetTest[, d + 1]) * nrow(dataSetTest)
```
Demean.
```{r demean with PCR, eval = F}
dataSetTrain[, d + 1] <- dataSetTrain[, d + 1] - 
  predict(pcrMdl, dataSetTrain[, 1 : d], ncomp = 20)
dataSetTest[, d + 1] <- dataSetTest[, d + 1] - 
  predict(pcrMdl, dataSetTest[, 1 : d], ncomp = 20)
dataSetTrainTest[, d + 1] <- dataSetTrainTest[, d + 1] - 
  predict(pcrMdl, dataSetTrainTest[, 1 : d], ncomp = 20)
rm(pcrMdl)
```
...
```{r EDA locs, eval = F}
i <- 9
ggplot(data.frame(x = c(locsTrain[, i], 
                        locsOOS[, i], locsTest[, i]),
                  y = c(yTrain, yOOS, yTest)), 
       aes(x = x, y = y)) + geom_point(size = 1)
```
Some EDA for principle components.
```{r EDA pc, eval = F}
pcObj <- prcomp(rbind(locsTrain, locsOOS, locsTest), rank. = 20)
i <- 1
ggplot(data.frame(x = pcObj$x[, i],
                  y = c(yTrain, yOOS, yTest)), 
       aes(x = x, y = y)) + geom_point(size = 1)
```
