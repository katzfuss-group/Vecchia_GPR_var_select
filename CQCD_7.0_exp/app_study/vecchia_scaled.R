...
fit_scaled=function(y,inputs,ms=c(30),trend='pre',X,nu=3.5,nug=0,scale='parms',
              ...
              n.est=min(5e3,nrow(inputs)),find.vcf=TRUE,vcf.scorefun=lsScore) {
...
predictions_scaled <- function(fit,locs_pred,m=100,joint=TRUE,nsims=0,X_pred){
  ...
  # specify how to scale input dimensions
  scales = sqrt(covparms[1+(1:ncol(locs_obs))])
  ...
  if(joint){  # joint predictions
    ...
    for(i in 1:n_pred){
      ...
      # prediction
      y.NN=y[NN_obs]
      LInv <- solve(cl[1:m,1:m])
      means[i]=cl[m+1,1:m] %*% LInv %*% c(y.NN,means[NN_pred])
      if(nsims>0){ # conditional simulation
        pred.var=cl[m+1,m+1]^2*vcf
        for(s in 1:nsims){
          pm=cl[m+1,1:m]%*% LInv %*%c(y.NN,samples[NN_pred,s])
...
### log score
lsScore=function(dat,mu,sig2) -mean(dnorm(dat,mu,sqrt(sig2),log=TRUE))
