                             ...
                             arg_check_SR, batchSzCQCD, batchSzGrad)
{
  ...
  while(T)
  {
    ...
    # break condition
    if(
      # (length(idxLocZero) > 0 && any(idxSel %in% idxDesel)) ||
       length(idx) > 100)
      break
  }
...
VReg <- function(m = 100, k = 5, outputFn = "VREG.RData"){
  lambdaUp <- 1
  lambdaVec <- c(rev(2^(seq(from = log2(1/16384), to = log2(max(0.1, lambdaUp)), 
                            by = 1))), 0)
  niter <- length(lambdaVec)
  batchSzCQCD <- min(5e3, nTrain)
  batchSzGrad <- min(5e3, nTrain)
