...
forward_backward <- function(theta0, idx0, gradObj0)
{
  ...
  while(T)
  {
    ...
    niterCQCD <- niterCQCD + optObj$neval
    timeTtCQCD <- timeTtCQCD + timeObj[[3]]
    ...
    timeObj <- system.time(
      gradObj <- 
        GpGp::vecchia_meanzero_loglik_grad_info(theta, 
                                                ...
                                                NNarray[batchIdx, , drop = F]))
    niterFB <- niterFB + 1
    timeTtGrad <- timeTtGrad + timeObj[[3]]
  }
  cat("#Iter of forward-backward selection is", niterFB, "\n")
  cat("#Iter of CQCD is", niterCQCD, "\n")
  cat("Time of computing the full gradients is", timeTtGrad, "\n")
  cat("Time of running CQCD is", timeTtCQCD, "\n")
  cat("Time of finding NN is", timeTtNN, "\n")
...
for(i in 1 : niter)
{
  ...
  scoreSet[[i]] <- optObj$OOS
  lambdaSet[[i]] <- lambda
  loglikSet[[i]] <- optObj$gradObj$loglik
  # OOS score
  cat("OOS is", scoreSet[[i]], "\n\n")
...
save(list = c("d", "idxSet", "thetaSet", "scoreSet", "lambdaSet", "loglikSet",
              "lambdaVec", "n"), 
     file = paste0("bridge_OOS_minibat_dep_", n, "_", d, ".RData"))
```
...
# ggsave(paste0("CQCD_bridge_minibat_dep_scr_", n, "_", d, ".pdf"), 
#        width = 7, height = 5)
...
ggplot(data = dfScoreChg, aes(x = lambda, y = scoreChg)) + 
  ...
  scale_x_continuous(trans = pseudo_log_trans(sigma = 0.125),
                     breaks = lambdaVec[seq(from = 1, 
                                            to = length(lambdaVec), by = 2)]) +
  theme(legend.position = "none")
# ggsave(paste0("CQCD_bridge_minibat_dep_scrChg_", n, "_", d, ".pdf"), 
#        width = 7, height = 5)
