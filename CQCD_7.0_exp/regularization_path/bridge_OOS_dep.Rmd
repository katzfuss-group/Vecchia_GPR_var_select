...
nSelected <- sapply(firstIdx : length(idxSet), function(x){if(x == firstIdx) 
  length(idxSet[[1]]) else length(setdiff(idxSet[[x]], idxSet[[x - 1]]))})
lambdaStop <- lambdaVec[which(dfScoreChg$scoreChg < 0.01 &
                                nSelected > 0)[1] + firstIdx - 2]
