---
title: "Bridge GP Regression with dependent predictors"
...
## Method 
Use OOS score and bridge penalty and stop if any newly added predictor is deselected in the same iteration. Dependent predictors are used.
...
covM <- matrix(rho, d, d)
diag(covM) <- 1
locs <- rmvnorm(n + nOOS, sigma = covM)
...
lambdaVec <- rev(c(0, 2^(seq(from = log2(0.125), to = log2(5000), by = 1))))
...
save(list = c("d", "idxSet", "thetaSet", "scoreSetLoglik", "scoreSetRMSE",
              ...
              "lambdaVec", "n"), 
     file = paste0("bridge_OOS_dep_", n, "_", d, ".RData"))
```

## Plot
```{r load data, eval = T}
load("bridge_OOS_dep_5000_1000.RData")
```
...
firstIdx <- max(which(sapply(thetaSet, 
                             function(x){sum(x[2 : (d + 1)]) > 1e-2}))[1] - 1, 
                1)
...
for(i in firstIdx : length(idxSet))
{
  ...
  dfScore[i - firstIdx + 1, 2] <- scoreSetLoglik[[i]]
}
...
{
  dfScore[i - firstIdx + 1, 1] <- lambdaSet[[i]]
  dfScore[i - firstIdx + 1, 2] <- scoreSetRMSE[[i]]
}
dfScore <- as.data.frame(dfScore)
...
ggplot(data = dfTheta, aes(x = lambda, y = relevance, col = varID)) +
  ...
  scale_color_manual(breaks = unique(dfThetaTrue$varID),
                     values = c(rainbow(5), rep("grey", d - 5))) +
  theme(legend.position = "none")
ggsave(paste0("CQCD_bridge_dep_", n, "_", d, ".pdf"), width = 7, height = 5)
```
...
{
  dfScore[i - firstIdx + 1, 1] <- lambdaSet[[i]]
  dfScore[i - firstIdx + 1, 2] <- scoreSetCRPS[[i]]
}
dfScore <- as.data.frame(dfScore)
