...
forward_backward <- function(theta0, idx0, gradObj0)
{
  ...
  while(T)
  {
    ...
    OOSNew <- OOS_score_CRPS(thetaNew)
    if(OOSNew > OOS * 0.99 || length(idx) > 20)
      break
    idx <- idxNew
    theta <- thetaNew
    OOS <- OOSNew
...
nSelected <- sapply(1 : length(idxSet), function(x){if(x == 1) 
