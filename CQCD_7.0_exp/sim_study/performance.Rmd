...
test_score <- function(theta, idxLocsRel)
{
  ...
  err <- predObj$means - yTest
  RMSE <- sqrt(mean(err^2))
  return(list(CRPScore = CRPScore, es = es, RMSE = RMSE))
}
```
...
cat(CRPS, es, RMSE, falseP, falseN, timeVal, sep = ",", file = dataOutFile, 
    append = T)
cat("\n", file = dataOutFile, append = T)
```
...
```{r read data, eval = F}
meaVec <- c("CRPS", "ES", "RMSE", "FPos", "FNeg", "Time")
depVec <- c("I", "D")
mtdVec <- c("VREG", "Fisher", "FWD", "PGPR")
...
library(patchwork)
rm(pltTtl)
for(mea in c("CRPS", "RMSE", "FPos", "Time")) # row
