...
if(dTrain <= 100){
  ...
  theta <- link_func(c(0.25, rep(0.01, dTrain), 0.01))
}
...
if(grepl("dep", dataInFile, ignore.case = T)){
  dataOutFile <- paste0("Fisher_scoring_dep_", nTrain, "_", dTrain, ".RData")
}else
  dataOutFile <- paste0("Fisher_scoring_", nTrain, "_", dTrain, ".RData")
